...
// 框架特殊字段，不会传递给组件
const FRAMEWORK_FIELDS = new Set(['#', '#view', '#children', '#content', '#if', '#show', '#style', '#class', '#events', '#staticProps'])

export default class EficyNode extends ObservableClass {
  // 唯一标识
  ...
  get props(): Record<string, any> {
    ...
    // 添加类名
    if (this['#class']) {
      props.className = Array.isArray(this['#class']) 
        ? this['#class'].join(' ') 
        : this['#class']
    }
