import { Action, Observable, ObservableClass, createAction, makeObservable, signal } from '@eficy/reactive';
...
describe('Integration Tests', () => {
  it('should work with signals and multiple state updates', () => {
    ...
    const App = observer(() => (
      <div>
        <h1 data-testid="name">{name.value}</h1>
  ...
  it('should work with multiple observers sharing same signal', () => {
    ...
    const Component1 = observer(() => <div data-testid="comp1">Component 1: {value.value}</div>);

    const Component2 = observer(() => <div data-testid="comp2">Component 2: {value.value * 2}</div>);
      ...
      return (
        <div>
          <div data-testid="stats">
            Total: {allTodos.length}, Active: {allTodos.filter((t) => !t.completed).length}, Filter: {currentFilter}
          </div>
