...
export interface ILifecyclePlugin extends IEficyPlugin {
  ...
  // 渲染钩子
  onRender?(EficyNode: EficyNode, context: IRenderContext, next: () => Promise<ReactElement>): Promise<ReactElement>;

  // 挂载钩子
  onMount?(element: Element, EficyNode: EficyNode, context: IMountContext, next: () => Promise<void>): Promise<void>;
